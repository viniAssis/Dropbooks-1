...
public class ProdutoDAO {
    ...
    public static String alterarProduto(Produto produto) {
     ...
     try {
         Connection con = Conecta.getConexao();
         String sql = "UPDATE produto SET titulo=?, autor=?, editora=?, dataPublicacao=?, genero=?, idioma=?, preco= ?, quantidade= ?, descricao=? WHERE id=?";
         PreparedStatement ps = con.prepareStatement(sql);
         
         ps.setString(1, produto.getTitulo());
         ps.setString(2, produto.getAutor());
         ps.setString(3, produto.getEditora());
         ps.setString(4, produto.getDataPublicacao().toString());
         ps.setString(5, produto.getGenero());
         ps.setString(6, produto.getIdioma());
